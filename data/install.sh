...
install_miniconda() {
    ...
    then
        echo "Deleting old Miniconda3-latest-Linux-x86_64.sh"
    ...
    if [ -f Miniconda3-latest-Linux-x86_64.sh ]
    then
        echo "Deleting Miniconda3-latest-Linux-x86_64.sh"
        rm -rf Miniconda3-latest-Linux-x86_64.sh
    fi
}
...
EOF
    ...
    certbot --nginx -d $DOMAIN -d api.$DOMAIN --register-unsafely-without-email
