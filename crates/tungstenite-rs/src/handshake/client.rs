...
use http::{
    header::HeaderName, HeaderMap, Request as HttpRequest, Response as HttpResponse, StatusCode, HeaderValue,
};
use httparse::Status;
use log::*;
...
pub fn generate_request(mut request: Request) -> Result<(Vec<u8>, String)> {
    ...
    write!(req, "Accept: */*\r\n").unwrap();
    write!(req, "Accept-Encoding: gzip, deflate\r\n").unwrap();
    write!(req, "Accept-Language: en-US,en;q=0.9\r\n").unwrap();
    write!(req, "Cache-Control: no-cache\r\n").unwrap();
    write!(req, "Pragma: no-cache\r\n").unwrap();
    write!(req, "Sec-Websocket-Extensions: permessage-deflate\r\n").unwrap();
    write!(req, "Sec-Websocket-Protocol: vite-hmr\r\n").unwrap();
    write!(req, "Origin: http://127.0.0.1:8080\r\n").unwrap();
    write!(req, "User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36\r\n").unwrap();
