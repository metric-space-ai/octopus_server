...
impl OctopusDatabase {
    ...
    pub async fn get_chat_messages_by_chat_id_and_status(
        ...
        let chat_messages = sqlx::query_as::<_, ChatMessage>(
            "SELECT id, ai_function_id, ai_function_call, chat_id, user_id, bad_reply_comment, bad_reply_is_harmful, bad_reply_is_not_helpful, bad_reply_is_not_true, bypass_sensitive_information_filter, estimated_response_at, is_anonymized, is_sensitive, message, progress, response, status, created_at, deleted_at, updated_at
            FROM chat_messages
            WHERE chat_id = $1
            AND status = $2
            AND deleted_at IS NULL
    ...
    pub async fn insert_chat_message(
        ...
        let chat_message = sqlx::query_as!(
            ...
            VALUES ($1, $2, $3, $4, $5)
            RETURNING id, ai_function_id, ai_function_call, chat_id, user_id, bad_reply_comment, bad_reply_is_harmful, bad_reply_is_not_helpful, bad_reply_is_not_true, bypass_sensitive_information_filter, estimated_response_at, is_anonymized, is_sensitive, message, progress, response, status AS "status: _", created_at, deleted_at, updated_at"#,
            chat_id,
            user_id,
    ...
    pub async fn update_chat_message(
        ...
        let chat_message = sqlx::query_as::<_, ChatMessage>(
            ...
            WHERE id = $1
            RETURNING id, ai_function_id, ai_function_call, chat_id, user_id, bad_reply_comment, bad_reply_is_harmful, bad_reply_is_not_helpful, bad_reply_is_not_true, bypass_sensitive_information_filter, estimated_response_at, is_anonymized, is_sensitive, message, progress, response, status, created_at, deleted_at, updated_at",
        )
        .bind(id)
        .bind(progress)
        .bind(response)
