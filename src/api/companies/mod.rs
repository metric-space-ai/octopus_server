...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Company read.", body = Company),
...
pub async fn read(
    ...
    let company = context
        .octopus_database
        .try_get_company_by_id(id)
        .await?
        .ok_or(AppError::NotFound)?;

    if session_user.company_id != id {
...
#[cfg(test)]
mod tests {
    use crate::{api, app, entity::Company};
    use axum::{
        body::Body,
    ...
    pub fn get_company_update_params() -> (String, String, String) {
        let address = Paragraph(1..2).fake::<String>();
        let custom_style = "custom_style".to_string();
    ...
    async fn read_200() {
        ...
        let body = BodyExt::collect(response.into_body())
            ...
            .to_bytes()
            .to_vec();
        let body: Company = serde_json::from_slice(&body).unwrap();

        assert_eq!(body.name, company_name);
    ...
    async fn read_404() {
        ...
        let second_company_id = "33847746-0030-4964-a496-f75d04499160";

        let response = router
            .oneshot(
                Request::builder()
                    .method(http::Method::GET)
                    .uri(format!("/api/v1/companies/{second_company_id}"))
                    .header(http::header::CONTENT_TYPE, mime::APPLICATION_JSON.as_ref())
                    .header("X-Auth-Token".to_string(), admin_session_id.to_string())
                    .body(Body::empty())
                    .unwrap(),
    ...
    async fn update_200() {
        ...
        let (address, custom_style, name) = get_company_update_params();
        let response = router
            .oneshot(
                Request::builder()
        ...
        assert_eq!(body.address, Some(address));
        assert_eq!(body.custom_style, Some(custom_style));
