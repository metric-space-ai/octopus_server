use crate::{
    context::Context,
    entity::{FileWithUrl, ROLE_COMPANY_ADMIN_USER},
    error::AppError,
...
pub async fn list(
    ...
    let files = context
        .octopus_database
        .get_files_by_user_id(session_user.id)
        .await?;
...
mod tests {
    ...
    pub async fn files_create(router: Router, session_id: Uuid) -> File {
        let body =
    ...
    async fn create_201() {
        ...
        let file = files_create(router, session_id).await;
        let file_id = file.id;
    ...
    async fn delete_204() {
        ...
        let file = files_create(router.clone(), session_id).await;
        let file_id = file.id;
