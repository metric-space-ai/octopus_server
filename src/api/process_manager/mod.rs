...
#[utoipa::path(
    get,
    path = "/api/v1/process-manager",
    responses(
        (status = 200, description = "Process manager list.", body = [Process]),
...
pub async fn list(
    ...
    let processes = context.process_manager.list_processes()?;

    Ok((StatusCode::OK, Json(processes)).into_response())
}

#[cfg(test)]
mod tests {
    use crate::{api, app, process_manager::Process};
    use axum::{
        body::Body,
